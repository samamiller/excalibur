...
let private headerCell text =
    TextBlock.create [ TextBlock.text text; TextBlock.fontWeight Avalonia.Media.FontWeight.Bold ] :> Types.IView

let private cell text = TextBlock.create [ TextBlock.text text ] :> Types.IView

let view (props: Props) : Types.IView =
    let row b : Types.IView =
        Grid.create [ Grid.columnDefinitions "2*,1.2*,1*,140,90"
                      Grid.children [
                          (cell b.title) |> Grid.column 0 :> Types.IView
                          (cell (defaultArg b.author "")) |> Grid.column 1 :> Types.IView
                          (cell (defaultArg b.tags "")) |> Grid.column 2 :> Types.IView
                          (cell (defaultArg b.added_at "")) |> Grid.column 3 :> Types.IView
                          (cell (if b.missing then "Yes" else "")) |> Grid.column 4 :> Types.IView
                      ]
    ...
    let header : Types.IView =
        Grid.create [ Grid.columnDefinitions "2*,1.2*,1*,140,90"
                      Grid.children [
                          (headerCell "Title") |> Grid.column 0 :> Types.IView
    ...
    StackPanel.create [ StackPanel.children (
                            [ header ] @ (props.Books |> List.map row)
                        ) ]
