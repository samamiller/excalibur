...
let view props : Types.IView =
    let searchControls: Types.IView list =
        [ TextBox.create
              [ TextBox.watermark "Search title/author"
                TextBox.text props.Query
                TextBox.onTextChanged props.OnQueryChanged
                TextBox.width 260.0 ]
          Button.create [ Button.content "Clear"; Button.onClick (fun _ -> props.OnClear()) ] ]

    let panelChildren: Types.IView list =
        [ StackPanel.create
              [ DockPanel.dock Avalonia.Controls.Dock.Top
                StackPanel.orientation Orientation.Horizontal
                StackPanel.spacing 8.0
                StackPanel.margin (Thickness 8.0)
                StackPanel.children searchControls ]
          StackPanel.create
              [ DockPanel.dock Avalonia.Controls.Dock.Left
                StackPanel.orientation Orientation.Horizontal
                StackPanel.spacing 6.0
                StackPanel.children
                    [ Button.create [ Button.content "Add book(s)"; Button.onClick (fun _ -> props.OnAddBooks()) ]
                      Button.create
          ...
          let rightChildren: Types.IView list =
              [ match props.Status with
                ...
                yield
                    (Button.create
                        [ Button.content (sprintf "%s" props.Theme)
                          Button.onClick (fun _ -> props.OnThemeToggle()) ]
                    :> Types.IView) ]

          StackPanel.create
              [ DockPanel.dock Avalonia.Controls.Dock.Right
                StackPanel.orientation Orientation.Horizontal
                StackPanel.spacing 8.0
                StackPanel.children rightChildren ] ]

    DockPanel.create [ DockPanel.lastChildFill true; DockPanel.children panelChildren ]
